    ...
    while step <= iteration_lvl3:
        for X, Y in training_generator:
            ...
            if (step % n_checkpoint == 0):
                ...
                # ---------------------------------------
                # OASIS (Validation)
                imgs = sorted(glob.glob(datapath + '/OASIS_OAS1_*_MR1/slice_norm.nii.gz'))[-4:]
                labels = sorted(glob.glob(datapath + '/OASIS_OAS1_*_MR1/slice_seg24.nii.gz'))[-4:]
                ...
                for batch_idx, data in enumerate(valid_generator):
                    X, Y, X_label, Y_label = data[0].squeeze(-1).to(device), data[1].squeeze(-1).to(device), data[
                        2].squeeze(-1).to(device), data[3].squeeze(-1).to(device)
